...
| --------------------------- | ---------- | --------------------------------- | ---------------------------- | --------------------- |
| next                        | `^16.3.0`  | 정적 웹사이트의 페이지·빌드 기능  | `src/app/`, `next.config.ts`     | 불가                  |
| react                       | `^19.2.8`  | 화면 컴포넌트 표시                | `src/app/`, `src/features/`, `src/components/` | 불가       |
| react-dom                   | `^19.2.8`  | React를 브라우저 화면에 연결      | Next.js 내부 화면 실행       | 불가                  |
| zustand                     | `^5.0.14`  | 사례 필터·페이지 번호 상태 관리   | `src/features/cases/store/`  | 가능                  |
...
| tailwindcss                 | `^4.3.3`   | 화면 스타일 작성                  | `src/app/globals.css`, TSX       | 가능                  |
| @tailwindcss/postcss        | `^4.3.3`   | Tailwind CSS 빌드 연결            | `postcss.config.mjs`         | Tailwind 사용 중 불가 |
| eslint                      | `^9.39.5`  | 코드 작성 실수 검사               | `config/eslint.config.mjs`          | 가능                  |
| eslint-config-next          | `^16.3.0`  | Next.js·React 검사 규칙           | `config/eslint.config.mjs`          | Next.js 사용 중 불가  |
| prettier                    | `^3.9.6`   | 코드 모양 통일                    | `config/prettier.config.mjs`        | 가능                  |
| prettier-plugin-tailwindcss | `^0.8.1`   | Tailwind 클래스 순서 통일         | `config/prettier.config.mjs`        | Tailwind 사용 중 가능 |
| husky                       | `^9.1.7`   | 커밋 직전 검사 실행               | `.husky/pre-commit`          | 가능                  |
| lint-staged                 | `^17.3.0`  | 이번 커밋의 변경 파일만 검사      | `package.json`, `.husky/`    | Husky 사용 중 가능    |
| vitest                      | `^4.1.10`  | 데이터 처리 규칙 자동 테스트      | `tests/`, `config/vitest.config.ts` | 가능                  |
