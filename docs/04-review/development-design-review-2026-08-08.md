...
- `docs/01-planning/PRD.md`, `content-model.md`, `implementation-decisions.md`
- `docs/02-design/design-system.md`
- `docs/03-development/`의 개발 구조·의존성·검증·오류·배포·통계·브라우저 지원 문서
...
| 디자인 인계 | 통과 | `styles/tokens.css`, `components/ui/`, `app/globals.css`로 디자인 시스템의 구현 위치가 명확하다. |
| 목록 상태 | 통과 | Zustand와 sessionStorage는 필터·페이지 상태에만 사용하며 URL 쿼리를 만들지 않는다. |
| 콘텐츠 분리 | 통과 | `content/` JSON, `public/images/`, 대표 이미지 위치, 여러 장 사진, 제품 참조 규칙이 기획과 일치한다. |
...
- 대상: `docs/03-development/project-structure.md`, `dependencies.md`, `deployment-plan.md`, DEC-030·DEC-035
...
- 대상: `docs/03-development/content-validation.md`, `docs/03-development/testing-policy.md`, REQ-006
